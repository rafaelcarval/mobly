...
function display_upload_form()
{
	?>
		<form action="uploader.php?<?php echo $_SERVER["QUERY_STRING"]; ?>&q=upload" method="post" enctype="multipart/form-data" onsubmit="">
			...
			<input type="file" size="40" name="upload_file" ID="File1"/>
			<br/><br/>
			<input type="submit" name="Upload File" value="Enviar" style="width: 90px;" onclick="document.getElementById('progress_div').style.visibility='visible';"/>			
...
function upload_content_file($DestPath, $DestLinkPath)
{
	...
	{		
		$StatusMessage = "Erro! Nenhuma imagem para enviar.";		
	}	
    ...
    if(!in_array(strtolower($ext), $allowedExtensions)){
        ...
        $StatusMessage = "Extensão inválida. Use apenas ". $these . ".";
    }
	
	if ($StatusMessage==="")
	{		
        //Overwrite here for safety
        //$DestPath = ""
        //$object_id = sha1($FileObject['tmp_name']);
        $object_id = "tempimage";
        $uploadedfile = $FileObject['tmp_name'];
        //$md5 = md5($uploadedfile);
        $md5 = md5(file_get_contents($uploadedfile));
        
        $ActualFileName = "{$DestPath}/{$object_id}_{$md5}.{$ext}";
		...
		{
			if($ReplaceFile=="yes")
				$StatusMessage .= "Atualizando arquivo existente. ";										// if so, we'll let the user know
			else
				$StatusMessage . "Error! A imagem '$ActualFileName' já existe no servidor!";
		}
		if($ReplaceFile=="yes")
		{
			if($ResizeSizeX!=0 || $ResizeSizeY!=0)														// if we need to resize the file
			{
				...
				{
					$StatusMessage .= "Não foi possível redimensionar a imagem. ";
					move_uploaded_file($FileObject['tmp_name'], $ActualFileName);
				}
			}
			else
				move_uploaded_file($FileObject['tmp_name'], $ActualFileName);
			$StatusMessage .=  "Imagem: " . $FileObject['name'] . " transmitida com sucesso.";		// to " . $ActualFileName . "!";						
			//$ActualFileName = $DestLinkPath . $FileObject['name'];										// now create the link to the specified link path
			$ActualFileName = "{$DestLinkPath}/{$object_id}_{$md5}.{$ext}";	
		}
	}	
