  ...
  tinymce.create('tinymce.plugins.MathquillPlugin', {
    ...
    init : function(ed, url) {
      ...
      ed.addCommand('mceMathquillInsert', function(latex) {
        if (!latex) return;
        var content = '<img class="simple-latex" '
      ...
      ed.onInit.add(function() {
        $(ed.getDoc()).on('click', 'img.simple-latex', function() {
          editing = this;
      ...
      ed.onPreProcess.add(function(ed, o) {
        if (o.get) {
          var mathquillimages = ed.dom.select('img.simple-latex', o.node);
          $(mathquillimages).replaceWith(function() {
            var latex = $(this).attr('alt');
            return '<span class="simple-latex">' + latexOpenTag + latex + latexCloseTag + '</span>';
          });
        } 
      });
	  ...
	  ed.onSetContent.add(function(ed,o) {
          var mathquills = ed.dom.select('span.simple-latex', o.node);
          $(mathquills).replaceWith(function() {
            ...
            var imgurl = '<img class="simple-latex" '
            	+ 'style="vertical-align:middle" '
            	+ 'src="' + latexImgRendererUrlTempl.replace('*', latex) + '" '
            	+ 'alt="' + latex + '"/>';
            return imgurl;
